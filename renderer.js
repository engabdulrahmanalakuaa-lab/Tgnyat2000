...
async function loadCompanyData() {
    try {
        ...
        alert('فشل تحميل بيانات الشركة: ' + error.message);
    }
}
...
async function openShiftIfNeeded() {
    try {
        ...
        alert('فشل في فتح الوردية: ' + error.message);
    }
}
...
async function switchTab(tab) {
    try {
        ...
        alert('حدث خطأ أثناء تبديل الصفحة: ' + error.message);
    }
}
...
async function renderDashboard() {
    try {
        ...
        document.getElementById('main-content').innerHTML = `
            ...
            <h3>آخر الطلبات</h3>
            <table><thead><tr><th>رقم</th><th>المبلغ</th><th>الضريبة</th><th>الإجمالي</th><th>طريقة الدفع</th><th>التاريخ</th></tr></thead><tbody>
            ${orders.slice(-5).map(o => `<tr><td>${o.id}</td><td>${o.total.toFixed(2)}</td><td>${(o.tax || 0).toFixed(2)}</td><td>${(o.total_with_tax || o.total).toFixed(2)}</td><td>${o.payment_method}</td><td>${o.time}</td></tr>`).join('')}
            </tbody></table>
        `;
    } catch (error) {
        console.error('خطأ في لوحة التحكم:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل لوحة التحكم: ' + error.message + '</div>';
    }
}
...
async function renderPOS() {
    try {
        ...
        document.getElementById('main-content').innerHTML = `
            ...
            <div class="pos-container">
                ...
                <div class="invoice-section">
                    ...
                    <div class="order-type-selector" style="display:flex; gap:10px; margin:8px 0;">
                        <button class="type-btn active" data-type="local" onclick="setOrderType('local')" style="flex:1; padding:8px; border:2px solid #e67e22; border-radius:6px; background:#fdf2e9; cursor:pointer; font-weight:700;">🏠 محلي</button>
                        <button class="type-btn" data-type="takeaway" onclick="setOrderType('takeaway')" style="flex:1; padding:8px; border:2px solid #ddd; border-radius:6px; background:white; cursor:pointer; font-weight:700;">🚗 سفري</button>
                    </div>
        ...
        `;
        // تحديث حالة أزرار النوع
        document.querySelectorAll('.type-btn').forEach(btn => {
            if (btn.dataset.type === currentOrderType) {
                btn.classList.add('active');
                btn.style.background = '#fdf2e9';
        ...
        console.error('خطأ في renderPOS:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل نقطة البيع: ' + error.message + '</div>';
    }
}
...
async function filterPOS(catId) {
    try {
        ...
        console.error('خطأ في filterPOS:', error);
...
async function searchPOS() {
    try {
        ...
        console.error('خطأ في searchPOS:', error);
        alert('فشل البحث: ' + error.message);
    }
}

// الدالة المعدلة لعرض الصور بشكل آمن
function renderPOSItems(products) {
    const grid = document.getElementById('pos-items-grid');
    if (!grid) {
        console.warn('لم يتم العثور على grid العناصر');
        return;
    }
    if (!products || products.length === 0) {
        grid.innerHTML = '<p style="text-align:center; color:gray; padding:20px;">لا توجد منتجات. أضف منتجات من قائمة المنتجات.</p>';
        return;
    }
    ...
    grid.innerHTML = products.map(p => {
        let imgTag = '';
        if (p.image) {
            const fullPath = `file://${userDataPath}/${p.image}`;
            imgTag = `<img src="${fullPath}" style="width:100%; height:80px; object-fit:cover; border-radius:4px;" onerror="this.style.display='none'">`;
        } else {
            imgTag = `<div style="width:100%; height:80px; background:#f0f0f0; display:flex; align-items:center; justify-content:center; border-radius:4px; font-size:24px;">🍽️</div>`;
        }
        return `
...
async function addToCartPOS(productId) {
    try {
        ...
        console.error('خطأ في addToCartPOS:', error);
...
async function checkoutPOS() {
    ...
    if (selectedPayment === 'cash') {
        const paid = prompt(`المجموع: ${subtotal.toFixed(2)} ر.س\nالضريبة: ${tax.toFixed(2)} ر.س\nالإجمالي: ${total.toFixed(2)} ر.س\nالمبلغ المدفوع:`, total.toFixed(2));
        if (paid === null) return;
    ...
    try {
        ...
        console.error('خطأ في checkoutPOS:', error);
        alert('حدث خطأ أثناء إنهاء الطلب: ' + error.message);
    }
}
...
async function printInvoice(orderId, items, subtotal, tax, total, paidAmount) {
    ...
    <body>
        <div class="receipt-header">
            ...
            <p>نوع الطلب: ${currentOrderType === 'local' ? 'محلي' : 'سفري'}</p>
        </div>
...
async function processRefund() {
    ...
    try {
        ...
        console.error('خطأ في processRefund:', error);
...
async function closeShift() {
    ...
    try {
        ...
        console.error('خطأ في closeShift:', error);
        alert('حدث خطأ أثناء إغلاق الوردية: ' + error.message);
    }
}
...
async function renderProducts() {
    try {
        ...
        console.error('خطأ في renderProducts:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل المنتجات: ' + error.message + '</div>';
    }
}
...
async function openProductModal(id = null) {
    ...
    try {
        ...
        console.error('خطأ في openProductModal:', error);
...
async function deleteProduct(id) {
    ...
    try {
        ...
        console.error('خطأ في deleteProduct:', error);
...
async function renderCategories() {
    try {
        ...
        console.error('خطأ في renderCategories:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل الأقسام: ' + error.message + '</div>';
    }
}
...
async function saveCategory() {
    ...
    try {
        ...
        console.error('خطأ في saveCategory:', error);
...
async function renderMaterials() {
    try {
        ...
        console.error('خطأ في renderMaterials:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل المواد: ' + error.message + '</div>';
    }
}
...
function openMaterialModal(id = null) {
    ...
    `;
    document.getElementById('modal').classList.add('active');
    // لو تعديل، تحميل البيانات (اختياري، لم يُنفذ بالكامل)
}
...
async function saveMaterial(id) {
    ...
    try {
        ...
        console.error('خطأ في saveMaterial:', error);
        alert('فشل حفظ المادة: ' + error.message);
    }
}
...
async function deleteMaterial(id) {
    ...
    try {
        ...
        console.error('خطأ في deleteMaterial:', error);
...
async function addStock(id) {
    ...
    try {
        ...
        console.error('خطأ في addStock:', error);
        alert('فشل إضافة المخزون: ' + error.message);
    }
}
...
async function renderTables() {
    try {
        ...
        console.error('خطأ في renderTables:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل الطاولات: ' + error.message + '</div>';
    }
}
...
async function saveTable() {
    ...
    try {
        ...
        console.error('خطأ في saveTable:', error);
        alert('فشل حفظ الطاولة: ' + error.message);
    }
}
...
async function deleteTable(id) {
    ...
    try {
        ...
        console.error('خطأ في deleteTable:', error);
        alert('فشل حذف الطاولة: ' + error.message);
    }
}
...
async function renderWaiters() {
    try {
        ...
        console.error('خطأ في renderWaiters:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل الكباتن: ' + error.message + '</div>';
    }
}
...
async function saveWaiter() {
    ...
    try {
        ...
        console.error('خطأ في saveWaiter:', error);
        alert('فشل حفظ الكابتن: ' + error.message);
    }
}
...
async function deleteWaiter(id) {
    ...
    try {
        ...
        console.error('خطأ في deleteWaiter:', error);
        alert('فشل حذف الكابتن: ' + error.message);
    }
}
...
async function generateReports() {
    ...
    try {
        ...
        console.error('خطأ في generateReports:', error);
        alert('فشل في تحميل التقارير: ' + error.message);
    }
}
...
async function renderExpenses() {
    try {
        ...
        console.error('خطأ في renderExpenses:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل المصروفات: ' + error.message + '</div>';
    }
}
...
async function saveExpense() {
    ...
    try {
        ...
        console.error('خطأ في saveExpense:', error);
...
async function deleteExpense(id) {
    ...
    try {
        ...
        console.error('خطأ في deleteExpense:', error);
...
async function renderAudit() {
    try {
        ...
        console.error('خطأ في renderAudit:', error);
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل سجل التدقيق: ' + error.message + '</div>';
    }
}
...
async function renderUsers() {
    try {
        ...
        document.getElementById('main-content').innerHTML = '<div class="alert-warning">حدث خطأ في تحميل المستخدمين: ' + error.message + '</div>';
    }
}
...
async function openUserModal(id = null) {
    try {
        ...
        console.error('خطأ في openUserModal:', error);
        alert('حدث خطأ: ' + error.message);
    }
}
...
async function toggleBlockUser(userId) {
    ...
    try {
        ...
        console.error('خطأ في toggleBlockUser:', error);
...
async function manualBackup() {
    try {
        ...
        console.error('خطأ في manualBackup:', error);
        alert('حدث خطأ أثناء النسخ الاحتياطي: ' + error.message);
    }
}
